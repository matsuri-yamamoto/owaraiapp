...
//                    self.reviewLabel.text = "レビュー" + "\n" + "編集"
        ...
        //ストックボタンのテキストをセット
        db.collection("stock").whereField("comedian_id", isEqualTo: self.comedianId).whereField("user_id", isEqualTo: currentUser?.uid).whereField("valid_flag", isEqualTo: true).getDocuments() {(querySnapshot, err) in
...
//                    self.comedianTypeLabel.text = document.data()["comedian_type"] as? String
                    ...
                    self.db.collection("review").whereField("comedian_id", isEqualTo: self.comedianId).whereField("private_flag", isEqualTo: false).getDocuments() {(querySnapshot, err) in
                        ...
                        if let err = err {
                            ...
                            if self.comedianCopyRight == "true" {
                                ...
                                if self.movieId2 != "" {
                                    ...
                                    self.contentViewHight.constant = CGFloat(200*reviewCount! + 395)
                            ...
                            if reviewCount! == 0 {
                                ...
                                noReviewLabel.frame = CGRect(x: 5, y: 5, width: 200, height: 50)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //likereviewをセット
        db.collection("like_review").whereField("review_id", isEqualTo: reviewId).whereField("like_flag", isEqualTo: true).getDocuments() {(querySnapshot, err) in
            if let err = err {
                print("Error getting documents: \(err)")
                return
